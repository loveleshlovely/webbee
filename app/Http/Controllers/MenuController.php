...
class MenuController extends BaseController
{
     ...
     * @param $parent
     * @return array
     * @method use for get children data
     */
    protected function getchild ($parent) {
        $childMenu = MenuItem::where('parent_id' , $parent)->get();
        if(count($childMenu) > 0) {
            foreach ($childMenu as $key => $getChildMenu) {
                $childMenu[$key]['children'] = $this->getchild($getChildMenu->id);
            }
        }
        return $childMenu;
    ...
    public function getMenuItems() {
        ...
        foreach($menu as $key => $getMenu) {
            $data = $this->getchild($getMenu->id); // Calling another method for get children data
            $menu[$key]['children'] = $data;
        }
        return $menu;
    }
}
